...
MEM_TEST(miniram, basic_operations) {
...
MEM_TEST(miniram, sequential_allocations) {
    miniram allocator(1024 * 1024 * 256);
...
MEM_TEST(miniram, reuse_and_fragmentation) {
    miniram allocator(1024 * 1024 * 256);
...
MEM_TEST(miniram, zero_fragmentation_scenario) {
...
MEM_TEST(miniram, query_methods) {
...
MEM_TEST(miniram, out_of_resources) {
    miniram allocator(1024);
    auto a = allocator.allocate(1024);
...
MEM_TEST(miniram, disjunct_copy_utility) {
...
MEM_TEST(miniram, defragmentation) {
    miniram ram(10240);
    std::vector<mini_alloc> allocations;
...
MEM_TEST(miniheap, post_defrag_integrity) {
    miniram ram(20480);
    std::vector<mini_alloc> allocations;
...
MEM_TEST(miniram, resize_operations) {
    miniram ram(1024);
