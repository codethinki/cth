...
export module cth.meta.ranges:decl;
...
export namespace cth::rng {

template<class Rng>
concept static_dim_rng = std::ranges::range<Rng> && !std::same_as<std::ranges::range_value_t<Rng>, cth::mta::rcvr_t<Rng>>;
...
concept viewable_rng = std::ranges::viewable_range<Rng>
    || (
        std::ranges::viewable_range<cth::mta::rcvr_t<Rng>>
        && requires(Rng rng) { cth::mta::rcvr_t<Rng>{rng}; }
    );
...
auto cxpr to_viewable(Rng&& rng) {
    if constexpr(std::ranges::viewable_range<Rng>) return std::forward<Rng>(rng);
        ...
        static_assert(requires() { cth::mta::rcvr_t<Rng>{rng}; }, "range must be view- or copyable");
        return cth::mta::rcvr_t<Rng>{rng};
    }
}
}
