...
export module cth.algorithm:views;
import :ranges;
...
struct drop_stride_fn {
    ...
    template<class Fn, class T, class U> requires (std::constructible_from<std::decay_t<T>, T> &&
        std::constructible_from<std::decay_t<U>, U>)
...
struct transform_call_fn {
    ...
    template<std::ranges::range Rng, class Call, class... Args> requires(std::is_invocable_v<Call,
        std::ranges::range_value_t<Rng>, Args...>)
