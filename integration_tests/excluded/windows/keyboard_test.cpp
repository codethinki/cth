...
namespace cth::keybd {
inline void callback(uint32_t key, uint32_t data) {
    std::println("callback: key \'{0}\', action: {1}", static_cast<char>(key), data);
}
inline void rawCallback(KBDLLHOOKSTRUCT const& key, WPARAM action) {
    std::cout << "rawCallback" << &key << &action << std::endl;
}
