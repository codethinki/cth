...
//    template<template<class> class Self, class U, class F>
//    constexpr auto transform(this Self<U>&& self, F&& f) {
//        constexpr bool inVoid = type::is_void<U>;
//        using internal_t = decltype(*std::forward<Self<U>>(self));
//
//        using raw_result_t = std::conditional_t<inVoid,
//            std::invoke_result_t<F>,
//            std::invoke_result_t<F, internal_t>
//        >;
//        using R = type::rcvr_t<raw_result_t>;
//
//        if(!self.has_value()) return Self<R>{std::nullopt};
//
//        if constexpr(type::is_void<R>) {
//            if constexpr(inVoid) std::invoke(std::forward<F>(f));
//            else std::invoke(std::forward<F>(f), *std::forward<Self<U>>(self));
//            return Self<R>{std::in_place};
//        }
//
//        if constexpr(inVoid) return Self<R>{std::invoke(std::forward<F>(f))};
//        return Self<R>{std::invoke(std::forward<F>(f), *std::forward<Self<U>>(self))};
//    }
//
//    template<template<class> class Self, class U, class F>
//    constexpr auto and_then(this Self<U>&& self, F&& f) {
//        constexpr bool inVoid = type::is_void<U>;
//        using internal_t = decltype(*std::forward<Self<U>>(self));
//
//        if(self.has_value()) {
//            if constexpr(inVoid) return std::invoke(std::forward<F>(f));
//            return std::invoke(std::forward<F>(f), *std::forward<Self<U>>(self));
//        }
//
//        using result_t = type::rcvr_t<std::conditional_t<inVoid,
//            std::invoke_result_t<F>,
//            std::invoke_result_t<F, internal_t>
//        >>;
...
//
//    template<class U = T> requires std::is_constructible_v<T, U&&> && (!std::is_same_v<std::decay_t<U>, optional>)
...
//    template<class... Args> constexpr T& emplace(Args&&... args) { return _value.emplace(std::forward<Args>(args)...); }
//
//    [[nodiscard]] constexpr bool has_value() const noexcept { return _value.has_value(); }
//    [[nodiscard]] constexpr explicit operator bool() const noexcept { return _value.has_value(); }
//
//    template<class Self> constexpr auto&& operator*(this Self&& self) { return *std::forward<Self>(self)._value; }
//    template<class Self> constexpr auto&& operator->(this Self&& self) { return std::forward<Self>(self)._value.operator->(); }
//
...
//    }
//
//    template<class U> constexpr T value_or(U&& def) const & { return _value.value_or(std::forward<U>(def)); }
//    template<class U> constexpr T value_or(U&& def) && { return std::move(_value).value_or(std::forward<U>(def)); }
//
//private:
...
//    [[nodiscard]] constexpr explicit operator bool() const noexcept { return _value != nullptr; }
//
...
//    }
//
//    template<class V> constexpr T value_or(V&& def) const {
//        if(_value) return *_value;
//        return static_cast<T>(std::forward<V>(def));
//    }
//
//private:
...
struct optional_base {
    ...
    constexpr auto transform(this Self&& self, F&& f) {
        ...
        using raw_result_t = type::conditional_trait_pack_t<
            inVoid,
