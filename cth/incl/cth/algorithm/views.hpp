...
struct to_ptr_range_fn : cth::ranges::piped_fn {
    ...
    requires std::is_lvalue_reference_v<std::ranges::range_reference_t<Rng>>
...
struct drop_stride_fn {
    ...
    requires(std::constructible_from<std::decay_t<T>, T> && std::constructible_from<std::decay_t<U>, U>)
...
struct transform_call_fn {
    ...
    template<std::ranges::range Rng, class Call, class... Args>
    requires(std::is_invocable_v<Call, std::ranges::range_value_t<Rng>, Args...>)
    ...
    template<class Call, class... Args, class Fn>
    requires(!std::ranges::range<Call>)
