...
requires std::derived_from<std::remove_cvref_t<Fn>, piped_fn>
...
struct pipe_call_closure : piped_fn {
    static_assert(
        cth::mta::all_satisfy<CPT(cth::mta::decayed), FnArgs...>,
        "the contained types must not be references"
    );

    template<class... CArgs>
    requires(std::same_as<std::decay_t<CArgs>, FnArgs> && ...)
