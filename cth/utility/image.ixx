...
export module cth.image;

import cth.io.log;

export namespace cth::img {


struct stb_image {

    stb_image(std::string const& path, int desired_channels) {
        CTH_STABLE_ERR(!std::filesystem::exists(path), "file not found") throw details->exception();

        int width = 0, height = 0, channels = 0;
        uint8_t* ptr = stbi_load(path.data(), &width, &height, &channels, desired_channels);

        CTH_STABLE_ERR(width == 0 || height == 0, "failed to load file") throw details->exception();
