...
template<type::arithmetic T>
[[nodiscard]] cxpr bool inRange(T x, T a, T b) {
    CTH_CRITICAL(a > b, "invalid input, a <= b required") {}
...
namespace bits {
    ...
    cxpr std::array<bool, sizeof(T) * 8> toBitArr(T val) {
        ...
        for(size_t i = 0; i < BIT_LENGTH; i++) bits[i] = ((static_cast<T>(1) << (BIT_LENGTH - 1 - i)) & val) > 0;
