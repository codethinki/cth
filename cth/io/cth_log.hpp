...
namespace dev {
     ...
     * \param expr (expression) == false -> code execution + delayed log message
...
#define CTH_DEV_DELAYED_LOG_TEMPLATE(severity, expr, message_str, ...) \
    if(bool const expression = static_cast<bool>(expr); expression) [[unlikely]]\
     if(auto const details =\
        (expression ? std::make_unique<cth::log::dev::LogObj<severity>>(cth::except::default_exception{severity, std::format(message_str, __VA_ARGS__),\
